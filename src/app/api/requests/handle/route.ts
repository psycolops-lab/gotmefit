...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!cur || !["admin","superadmin"].includes(cur.role)) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    ...
    // Approve
    if (cur.role === "admin" && doc.gym && String(doc.gym) !== String(cur.gym)) return NextResponse.json({ error: "Forbidden" }, { status: 403 });
