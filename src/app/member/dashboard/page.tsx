  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (mp?.assigned_trainer_id) {
        // FIXED: Use the new server-side API route instead of direct query
        const response = await fetch(`/api/member/trainer?trainer_id=${mp.assigned_trainer_id}`, {
          headers: {
        ...
        if (response.ok) {
          ...
          if (result.trainer_name) {
            setTrainer({ full_name: result.trainer_name });
          }
        } else {
          console.error("Trainer API error:", await response.json());
        }
      }
